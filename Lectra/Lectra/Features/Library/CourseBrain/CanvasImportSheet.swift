...
struct CanvasImportSheet: View {
    ...
    let showSelectionOverride: Bool
            ...
            content
                .background(
                    ZStack {
                        ...
                        LectraGradient.appBackdrop.opacity(0.7)
                    }
                )
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 6) {
            Text("Pick courses to back up")
                ...
                .foregroundColor(LectraColor.textPrimary)
            Text("Canvascope imports every PDF from each selected course's assignments and Files tab into Imported From Canvas.")
        ...
        return Button {
            ...
            HStack(alignment: .center, spacing: 14) {
                Image(systemName: isSelected ? "checkmark.circle.fill" : "circle")
                    ...
                    .foregroundColor(isSelected ? LectraColor.accentSoft : LectraColor.textTertiary)
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 14, style: .continuous)
                    .stroke(isSelected ? LectraColor.accentSoft.opacity(0.55) : LectraColor.edgeStroke, lineWidth: 1)
            )
        }
        .buttonStyle(.plain)
    }
                    ...
                    Text(selectedCourseIDs.isEmpty ? "Import" : "Import \(selectedCourseIDs.count) Course\(selectedCourseIDs.count == 1 ? "" : "s")")
                        ...
                        .background(
                            RoundedRectangle(cornerRadius: 12, style: .continuous)
                                .fill(selectedCourseIDs.isEmpty ? LectraColor.surfaceFloating.opacity(0.82) : LectraColor.accent)
                        )
                }
                .buttonStyle(.plain)
    ...
    private var progressBody: some View {
        VStack(spacing: 24) {
            ...
            ZStack {
                Circle()
                    .stroke(LectraColor.edgeStroke, lineWidth: 8)
                Circle()
                    .trim(from: 0, to: progressFraction)
                    .stroke(LectraColor.accentSoft, style: StrokeStyle(lineWidth: 8, lineCap: .round))
    ...
    private func summaryBody(imported: Int, skipped: Int, failed: Int, cancelled: Bool) -> some View {
        VStack(spacing: 18) {
            ...
            Image(systemName: cancelled ? "stop.circle" : (failed > 0 ? "exclamationmark.triangle.fill" : "checkmark.seal.fill"))
                ...
                .foregroundColor(cancelled ? LectraColor.warning : (failed > 0 ? LectraColor.warning : LectraColor.accentSoft))
            ...
            VStack(spacing: 6) {
                summaryRow(label: "Imported", value: imported, color: LectraColor.accentSoft)
                if skipped > 0 {
                    summaryRow(label: "Already in Canvascope", value: skipped, color: LectraColor.textSecondary)
                }
                if failed > 0 {
    ...
    private func countPDFResources(in course: CourseTwin) -> Int {
        ...
        for resource in course.resources {
            ...
            guard let sourceURL = CanvasFileURLResolver.pdfSourceURL(
                from: url,
                title: resource.title,
                contentType: resource.contentType
            ) else { continue }
            seen.insert(sourceURL.absoluteString.lowercased())
        }

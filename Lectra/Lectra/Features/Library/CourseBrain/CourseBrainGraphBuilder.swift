...
final class CourseBrainGraphBuilder {
    ...
    private func mergeNodes(existing: CourseBrainNode, incoming: CourseBrainNode) -> CourseBrainNode {
        let mergedMetadata = CourseBrainNodeMetadata(
            ...
            assignmentId: existing.metadata.assignmentId ?? incoming.metadata.assignmentId,
            ...
            submitted: existing.metadata.submitted ?? incoming.metadata.submitted,
            submissionStatus: existing.metadata.submissionStatus ?? incoming.metadata.submissionStatus,
            submissionSummary: existing.metadata.submissionSummary ?? incoming.metadata.submissionSummary,
    ...
    private func buildLeafSummaries(from leaves: [CourseBrainNode]) -> [String: CourseBrainLeafSummary] {
        ...
        for leaf in leaves {
            result[leaf.id] = CourseBrainLeafSummary(
                ...
                assignmentId: leaf.metadata.assignmentId,
                dueAt: leaf.metadata.dueAt,
                unlockAt: leaf.metadata.unlockAt,
                lockAt: leaf.metadata.lockAt,
                url: leaf.resourceURL,
                submitted: leaf.metadata.submitted,
                submissionStatus: leaf.metadata.submissionStatus,
                submissionSummary: leaf.metadata.submissionSummary,
